#!/usr/bin/env bash
#SBATCH --job-name=cogni_adresso_af
...
CONFIG_FILE="modules/configs/default.yaml"

echo "Starting CogniAlign+AF ADReSSo training with config: $CONFIG_FILE"
echo "use_acoustic_features: $(grep use_acoustic_features $CONFIG_FILE)"

python -u modules/main.py --config "$CONFIG_FILE"
