...
def extract_features(audio_path, sr=22050, frame_length=2048, hop_length=512, silence_threshold=0.1):
    # Load audio with librosa (handles .alac via ffmpeg)
    ...
    # Speaking Rate via Onset Envelope
    ...
    # Parselmouth Features (requires file path)
    temp_wav = None
    if not audio_path.lower().endswith('.wav'):
        # Convert to temp wav for parselmouth
...
def process_audio_files(data_path, output_csv, sr, frame_length, hop_length, silence_threshold):
    ...
    for root, _, files in os.walk(data_path):
        for file in files:
            if not (file.lower().endswith('.wav') or file.lower().endswith('.alac')):
                continue
            
            audio_path = os.path.join(root, file)
            # Clean file_id logic to match dataset.py
            ...
            try:
                ...
                features['class'] = os.path.basename(root) # Might be 'audio' or 'dx' folder, but file_id is key
...
def main():
    ...
    parser.add_argument('data_path', help='Path to the directory containing audio files.')
    parser.add_argument('--output_csv', default='acoustic_features.csv', help='Output CSV file path.')
    ...
    parser.add_argument('--silence_threshold', type=float, default=0.1, help='Silence threshold as a fraction.')
