...
from amyloid.dataset import get_dataloaders, set_splits
...
def set_up(config, train_dataloader, device, fold=0):
    ...
    wandb_project = os.environ.get('WANDB_PROJECT', config.wandb.project if hasattr(config, 'wandb') else 'CogniAligned-Amyloid')
        ...
        with open(log_file, "w") as log:
            for fold in range(config.train.cross_validation_folds):
                ...
                if not os.path.exists(os.path.join(config.data.splits_path, f'val_uids{fold}.npy')):
                    print(f"Split file for fold {fold} missing. Regenerating splits...")
                    set_splits(config)
...
if __name__ == '__main__':
    ...
    save_config(config)
    config.path_name = f"amyloid_{config.path_name}"
    main(config)
